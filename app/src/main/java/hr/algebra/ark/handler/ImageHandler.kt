...
fun downloadImageAndStore(context: Context, urlImage: String): String? {
    val filename = urlImage.substring(urlImage.lastIndexOf("/") + 1)
    val file: File = createFile(context, filename)

    try {
        val con: HttpURLConnection = createGetHttpUrlConnection(urlImage)
        Files.copy(con.inputStream, Paths.get(file.toURI()))
        return file.absolutePath
    } catch (e: java.lang.Exception) {
        Log.e("IMAGES_HANLDER", e.toString(), e)
    }
